...
def forget(request):
    if request.method == 'POST':
        ...
        key = request.POST['key']
        customer_data = Customers.objects.filter(user__username=username)
...
def add_to_cart(request, id):
    ...
    if request.user.is_authenticated:
        ...
        if not created:
            ...
            # Set the price of the cart item to the product's price
            cart_item.price = product.price

        cart_item.save()

        # Update the total quantity and total price in the user's cart
        user_cart.total_quantity += 1
        user_cart.total_price += product.price
        user_cart.save()

        return ProductView(request, id)  # Redirect to the cart view
    else:
        # Handle the case where the user is not authenticated (you can implement your own logic)
...
def move_all_to_cart(request):
    ...
    for wishlist_item in wishlist_items:
        ...
        except user_cart.DoesNotExist or cart_item.DoesNotExist:
            # If the user doesn't have a cart, create one and add the product
            if user_cart.DoesNotExist:
